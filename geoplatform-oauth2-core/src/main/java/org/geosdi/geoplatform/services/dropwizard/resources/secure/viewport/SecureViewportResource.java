 ...
 */
public interface SecureViewportResource extends GPViewportResource {

    GPViewport getDefaultViewport(Principal principal, Long accountProjectID)
            throws Exception;

    WSGetViewportResponse getAccountProjectViewports(Principal principal,
            Long accountProjectID) throws Exception;

    Long insertViewport(Principal principal,
            InsertViewportRequest insertViewportReq) throws Exception;

    Long updateViewport(Principal principal, GPViewport viewport)
            throws Exception;

    GPViewport getViewportById(Principal principal, Long idViewport)
            throws Exception;

    Boolean deleteViewport(Principal principal, Long viewportID)
            throws Exception;

    void saveOrUpdateViewportList(Principal principal,
            ManageViewportRequest request) throws Exception;

    void replaceViewportList(Principal principal, ManageViewportRequest request)
            throws Exception;
}
